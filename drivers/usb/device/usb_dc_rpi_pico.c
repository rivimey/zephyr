...
LOG_MODULE_REGISTER(usb_dc_rpi_pico);

#define DT_DRV_COMPAT rpi_pico_usbd
...
/* Endpoint state */
struct usb_dc_rpi_pico_ep_state {
...
/* Driver state */
struct usb_dc_rpi_pico_state {
	usb_dc_status_callback status_cb; /* Status callback */
	struct usb_dc_rpi_pico_ep_state out_ep_state[USB_NUM_BIDIR_ENDPOINTS];
	struct usb_dc_rpi_pico_ep_state in_ep_state[USB_NUM_BIDIR_ENDPOINTS];
	bool setup_available;
	bool should_set_address;
	uint8_t addr;
};

static struct usb_dc_rpi_pico_state usb_dc_rpi_pico_state;

/* Internal functions */

static struct usb_dc_rpi_pico_ep_state *usb_dc_rpi_pico_get_ep_state(uint8_t ep)
{
	struct usb_dc_rpi_pico_ep_state *ep_state_base;
	...
	if (USB_EP_DIR_IS_OUT(ep)) {
		ep_state_base = usb_dc_rpi_pico_state.out_ep_state;
	} else {
		ep_state_base = usb_dc_rpi_pico_state.in_ep_state;
	}

	return ep_state_base + USB_EP_GET_IDX(ep);
}

static int usb_dc_rpi_pico_start_xfer(uint8_t ep, const void* data, size_t len)
{
	struct usb_dc_rpi_pico_ep_state* ep_state = usb_dc_rpi_pico_get_ep_state(ep);
...
static void usb_dc_rpi_pico_handle_setup(void)
{
	struct usb_dc_rpi_pico_ep_state* ep = usb_dc_rpi_pico_get_ep_state(EP0_OUT);
	LOG_DBG("");

	usb_dc_rpi_pico_state.setup_available = true;

	/* Reset PID to 1 for EP0 IN */
	usb_dc_rpi_pico_get_ep_state(EP0_IN)->next_pid = 1;

	ep->cb(EP0_OUT, USB_DC_EP_SETUP);
}

static void usb_dc_rpi_pico_handle_buff_status(void)
{
	struct usb_dc_rpi_pico_ep_state *ep;
	...
	for (i = 0U; status && i < USB_NUM_BIDIR_ENDPOINTS * 2; i++) {
		if (status & bit) {
			...
			ep = usb_dc_rpi_pico_get_ep_state(ep_addr);
			status_code = in ? USB_DC_EP_DATA_IN : USB_DC_EP_DATA_OUT;

			LOG_DBG("buff ep %i in? %i", (i >> 1), in);

			if (i == 0 && in && usb_dc_rpi_pico_state.should_set_address)
			{
				usb_dc_rpi_pico_state.should_set_address = false;
				usb_hw->dev_addr_ctrl = usb_dc_rpi_pico_state.addr;
			}
...
static void usb_dc_rpi_pico_isr(const void *arg)
{
	...
	if (status & USB_INTS_BUFF_STATUS_BITS) {
		...
		usb_dc_rpi_pico_handle_buff_status();
	}

	// Connection status update
	if (status & USB_INTS_DEV_CONN_DIS_BITS) {
		LOG_DBG("buf %u ep %u", *usb_dc_rpi_pico_get_ep_state(0x81)->buffer_control, *usb_dc_rpi_pico_get_ep_state(0x81)->endpoint_control);
		handled |= USB_INTS_DEV_CONN_DIS_BITS;
		usb_hw_clear->sie_status = USB_SIE_STATUS_CONNECTED_BITS;
		usb_dc_rpi_pico_state.status_cb(usb_hw->sie_status & USB_SIE_STATUS_CONNECTED_BITS ? USB_DC_CONNECTED : USB_DC_DISCONNECTED, NULL);
	}
	...
	if (status & USB_INTS_BUS_RESET_BITS) {
		...
		for (i = 0; i < USB_NUM_BIDIR_ENDPOINTS; i++) {
			k_sem_give(&usb_dc_rpi_pico_state.in_ep_state[i].write_sem);
		}

		usb_dc_rpi_pico_state.status_cb(USB_DC_RESET, NULL);
	}
...
void usb_dc_rpi_pico_init_bidir_endpoint(uint8_t i)
{
	usb_dc_rpi_pico_state.out_ep_state[i].buffer_control = &usb_dpram->ep_buf_ctrl[i].out;
	usb_dc_rpi_pico_state.in_ep_state[i].buffer_control = &usb_dpram->ep_buf_ctrl[i].in;

	if (i != EP0_IDX) {
		usb_dc_rpi_pico_state.out_ep_state[i].endpoint_control = &usb_dpram->ep_ctrl[i - 1].out;
		usb_dc_rpi_pico_state.in_ep_state[i].endpoint_control = &usb_dpram->ep_ctrl[i - 1].in;

		usb_dc_rpi_pico_state.out_ep_state[i].data_buffer = &usb_dpram->epx_data[((i - 1) * 2 + 1) * DATA_BUFFER_SIZE];
		usb_dc_rpi_pico_state.in_ep_state[i].data_buffer = &usb_dpram->epx_data[((i - 1) * 2) * DATA_BUFFER_SIZE];
	} else {
		usb_dc_rpi_pico_state.out_ep_state[i].data_buffer = &usb_dpram->ep0_buf_a[0];
		usb_dc_rpi_pico_state.in_ep_state[i].data_buffer = &usb_dpram->ep0_buf_a[0];

	}

	k_sem_init(&usb_dc_rpi_pico_state.in_ep_state[i].write_sem, 1, 1);
}

static int usb_dc_rpi_pico_init(void)
{
	unsigned int i;
	...
	for (i = 0U; i < USB_NUM_BIDIR_ENDPOINTS; i++) {
		usb_dc_rpi_pico_init_bidir_endpoint(i);
	}
	
	IRQ_CONNECT(USB_IRQ, USB_IRQ_PRI,
		    usb_dc_rpi_pico_isr, 0, 0);
	irq_enable(USB_IRQ);
...
void usb_dc_set_status_callback(const usb_dc_status_callback cb)
{
	...
	usb_dc_rpi_pico_state.status_cb = cb;
}
...
int usb_dc_set_address(const uint8_t addr)
{
	...
	usb_dc_rpi_pico_state.should_set_address = true;
	usb_dc_rpi_pico_state.addr = addr;
...
int usb_dc_ep_start_read(uint8_t ep, size_t len)
{
	...
	ret = usb_dc_rpi_pico_start_xfer(ep, NULL, len);
...
static inline uint32_t usb_dc_ep_rpi_pico_buffer_offset(volatile uint8_t *data_buffer)
{
	...
	*ep_state->buffer_control = (USB_EP_GET_IDX(ep) == 0 ? USB_BUF_CTRL_DATA1_PID : 0);
	...
	if (ep_state->endpoint_control)
	{
		uint32_t val = EP_CTRL_ENABLE_BITS
			...
			| usb_dc_ep_rpi_pico_buffer_offset(ep_state->data_buffer);
	...
	{
		usb_dc_rpi_pico_start_xfer(ep, NULL, ep_state->ep_mps);
	}
	#endif
		    ...
		    const uint32_t data_len, uint32_t * const ret_bytes)
{
	...
	ret = usb_dc_rpi_pico_start_xfer(ep, data, len);
...
uint32_t usb_dc_rpi_pico_get_ep_buffer_len(const uint8_t ep)
{
	struct usb_dc_rpi_pico_ep_state *ep_state = usb_dc_rpi_pico_get_ep_state(ep);
			...
			uint32_t *read_bytes)
{
	...
	if (usb_dc_rpi_pico_state.setup_available)
	{
		read_count = SETUP_SIZE;
	}
	else
	{
		read_count = usb_dc_rpi_pico_get_ep_buffer_len(ep) - ep_state->read_offset;
	}
...

{
	...
	size_t bytes_received = (usb_dc_rpi_pico_state.setup_available ?
		SETUP_SIZE : usb_dc_rpi_pico_get_ep_buffer_len(ep));

	usb_dc_rpi_pico_state.setup_available = false;
